#!/bin/bash
#
# test-external-connection.sh - Test external connection to Redis Enterprise Database
#
...
echo "=== Testing External Connection to Redis Enterprise Database ==="
echo ""
...
echo "Command: redis-cli -h $LB_HOSTNAME -p $DB_PORT --tls --insecure --sni $DB_NAME.$LB_HOSTNAME -a $PASSWORD"
echo ""
echo "Executing command..."
redis-cli -h "$LB_HOSTNAME" -p "$DB_PORT" --tls --insecure --sni "$DB_NAME.$LB_HOSTNAME" -a "$PASSWORD" PING
...
if [ $? -eq 0 ]; then
    ...
    if nc -z -w 5 "$LB_HOSTNAME" "$DB_PORT" &>/dev/null; then
        echo "Port $DB_PORT is open on $LB_HOSTNAME."
    else
        echo "Error: Port $DB_PORT is not open on $LB_HOSTNAME."
    ...
    if openssl s_client -connect "$LB_HOSTNAME:$DB_PORT" -servername "$DB_NAME.$LB_HOSTNAME" -quiet </dev/null &>/dev/null; then
    ...
    if openssl s_client -connect "$LB_HOSTNAME:$DB_PORT" -servername "$DB_NAME.$LB_HOSTNAME" -quiet </dev/null 2>&1 | grep -q "Server name"; then
        echo "SNI is working."
    else
