#!/bin/bash
#
# test-conn-replicaof.sh - Test connection to Redis Enterprise Replica Database from inside a pod
#
# This script tests the connection to the Redis Enterprise Replica Database from inside a Redis Enterprise pod.
...
echo "Testing connection to Redis Enterprise Replica Database '$DB_NAME' from inside a pod..."
...
if [ -z "$DB_PORT" ]; then
    ...
    if [ -z "$DB_PORT" ]; then
        echo "Warning: Could not get port from database status. Using the port specified in the database spec..."
        DB_PORT=$(kubectl get redb "$DB_NAME" -n "$NAMESPACE" -o jsonpath='{.spec.databasePort}' 2>/dev/null)
        if [ -z "$DB_PORT" ]; then
...
if [ -z "$PASSWORD" ]; then
    echo "Warning: Could not retrieve the database password from the replicaof secret."
    echo "Trying to get the password from the primary-replica-source secret..."
    PASSWORD=$(kubectl get secret "primary-replica-source" -n "$NAMESPACE" -o jsonpath="{.data.password}" | base64 --decode 2>/dev/null)
    if [ -z "$PASSWORD" ]; then
        echo "Warning: Could not retrieve the database password."
        echo "You may need to provide the password manually when connecting."
        AUTH_OPTION=""
    else
        echo "Database password retrieved from primary-replica-source secret."
...
echo ""
echo "Connecting to Redis Enterprise Replica Database from inside pod $POD_NAME..."
echo ""
...
echo "2. Make sure the replica database is active and accessible"
echo "3. Make sure you have the correct permissions to access the database"
echo "4. Make sure the replication is working correctly"
echo ""
